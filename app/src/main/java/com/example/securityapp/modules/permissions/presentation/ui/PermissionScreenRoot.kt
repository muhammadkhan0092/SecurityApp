...
fun PermissionScreenRoot(
    ...
    LaunchedEffect(Unit){
        vm.events.collectLatest {
            when(it){
                is PermissionEvent.RequestOtherPermission -> {
                    launcher.launch(it.permissions)
                }
                ...
                is PermissionEvent.RequestBackgroundLocationPermission -> backgroundLocationLauncher.launch(it.permissions)
            }
        }
    }
