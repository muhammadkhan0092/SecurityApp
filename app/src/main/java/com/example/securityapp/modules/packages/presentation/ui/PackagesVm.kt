package com.example.securityapp.modules.packages.presentation.ui

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.example.securityapp.core.domain.utils.Result
import com.example.securityapp.modules.packages.domain.PackageRepository
import com.example.securityapp.modules.packages.domain.PackagesComplete
import com.example.securityapp.modules.packages.presentation.models.PackagesAction
import com.example.securityapp.modules.packages.presentation.models.PackagesEvent
import com.example.securityapp.modules.packages.presentation.models.PackagesState
import com.example.securityapp.modules.uninstall.domain.UninstallDomainModel
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.Job
import kotlinx.coroutines.delay
import kotlinx.coroutines.flow.MutableSharedFlow
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.asSharedFlow
import kotlinx.coroutines.flow.asStateFlow
import kotlinx.coroutines.flow.update
import kotlinx.coroutines.launch
import javax.inject.Inject

@HiltViewModel
class PackagesVm @Inject constructor(
    private val packagesComplete: PackagesComplete,
    private val packagesRepository: PackageRepository,
    private val reducer: PackagesReducer
) : ViewModel() {
    private val _state = MutableStateFlow(PackagesState())
    val state = _state.asStateFlow()
    private val _events = MutableSharedFlow<PackagesEvent>(replay = 0, extraBufferCapacity = 1)
    val events = _events.asSharedFlow()
    var job: Job? = null
    fun onAction(action: PackagesAction) {
        when (action) {
            PackagesAction.OnNextClicked -> {
                viewModelScope.launch(Dispatchers.IO) {
                    if (state.value.selectedPackages.isEmpty()) {
                        _events.emit(PackagesEvent.Toast("Select Atleast One App"))
                        return@launch
                    }
                    val selectedPackages = state.value.fullPackageModel.filter { pack ->
                        state.value.selectedPackages.any { pack.name == it }
                    }
                    val result = packagesComplete(
                        selectedPackages.map {
                            UninstallDomainModel(it.packageName)
                        }
                    )
                    when (result) {
                        is Result.Error<*> -> {
                            _events.emit(PackagesEvent.Toast(result.error))
                        }

                        is Result.Success -> _events.emit(PackagesEvent.NavigateToControlledMain)
                    }
                }
            }

            is PackagesAction.OnPackageClicked -> {
                val newState = reducer.onPackageClicked(action, state.value)
                _state.update {
                    newState
                }
            }

            is PackagesAction.OnTextChanged -> {
                _state.update {
                    it.copy(etValue = action.text)
                }
                job?.cancel()
                job = viewModelScope.launch(Dispatchers.IO) {
                    delay(1000)
                    val newState = reducer.onQueryChanged(action, state.value)
                    _state.update {
                        newState
                    }
                }
            }
        }
    }

    init {
        val list = packagesRepository.getInstalledApps()
        _state.update {
            it.copy(allPackages = list.map {
                it.name
            },
                fullStringList = list.map {
                    it.name
                },
                fullPackageModel = list
            )
        }
    }
}